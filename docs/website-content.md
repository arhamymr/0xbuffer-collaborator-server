...
### Confirm Hard-to-See Findings
Track callbacks from systems that respond out of band, including web requests and DNS lookups.

### Keep Evidence Organized
Store generated payloads and captured interactions in one place for later review.
...
1. Create a unique collaborator payload.
2. Use the payload during a security test.
3. Wait for the target system to make a callback.
4. Review the captured HTTP or DNS interaction.
5. Save the evidence for reporting or follow-up.

## Use Cases

### Blind Vulnerability Testing
Detect behavior that does not appear in the direct response, such as delayed server-side requests.

### Payload Tracking
Create unique addresses for tests so each callback can be connected to the right finding.

### Evidence Collection
Record callback timing, request details, and interaction history for cleaner validation.

### Internal Security Labs
Run a private collaborator service for training, research, and controlled testing environments.
...
Because the service is self-hosted, your team controls the deployment, domain, access, and data storage. This is especially useful for organizations that need tighter handling of security testing evidence.

## Website Section Ideas

### Home
Introduce the product, explain the problem it solves, and guide visitors toward setup or documentation.

### How It Works
Show the payload-to-callback workflow in a simple visual sequence.

### Features
Highlight callback collection, payload tracking, self-hosting, HTTP and DNS listeners, and evidence review.

### Deployment
Explain the high-level setup path without overwhelming new users with commands.

### Documentation
Link to detailed installation, configuration, and usage guides.

## Feature Copy

### HTTP Callback Collection
Capture inbound web requests created by your test payloads.

### DNS Interaction Tracking
Record DNS lookups that can reveal blind or background behavior.

### Payload Management
Create and organize unique payloads for different tests and targets.

### Interaction History
Review captured activity later, even after a test session ends.

### Private Infrastructure
Keep collaborator traffic and evidence inside systems you control.

### Production-Friendly Deployment
Run with Docker Compose and publish through a standard reverse proxy setup.
